...
def generate(doc, ctx: Ctx, docket_cfg: Dict[str, Any], out: Out
    ...
    _import_wall_frame(doc, ctx, docket_cfg, out, result)
    return result
...
def _import_wall_frame(doc, ctx: Ctx, docket_cfg: Dict[str, Any], out: Out,
                       result: DocketResult) -> None:
    """Copy the configured wall-frame layers verbatim into the output.

    Absence of configured layers means "no wall frame copied" — never guessed
    from magic layer names.
    """
    wf = docket_cfg.get("wall_frame") or {}
    layers = list(wf.get("mirror_layers") or []) + \
        list(wf.get("hatch_layers") or [])
    if not layers:
        return
    wanted = {l.strip().lower()
              for l in ctx.resolver.match_layers(layers)}
    if not wanted:
        return
    ...
    def keep(e) -> bool:
        ...
        if layer not in wanted:
            return False
        if e.dxftype() == "INSERT":
            if layer in excl_layers:
                return False
            if e.dxf.name.strip().lower() in excl_blocks:
                return False
        return True
    ...
    # Wall geometry is often nested inside block references (the INSERT sits
    # on an unrelated layer while its contents carry the wall layers).  Walk
    # every INSERT that was NOT imported verbatim and pull out the entities
    # on the wanted layers as world-space copies.
    ...
    def explode(insert, visited) -> None:
        ...
        if name in visited:
            result.warn(f"cyclic block reference detected at '{name}' — "
                        f"recursion stopped")
            return
        if name.strip().lower() in excl_blocks:
            return
        try:
            layer = (insert.dxf.layer or "").strip().lower()
        except Exception:
            layer = ""
        if layer in excl_layers:
            return
        ...
        for ve in virtual:
            if ve.dxftype() == "INSERT":
                ...
                continue
            try:
                ve_layer = (ve.dxf.layer or "").strip().lower()
            except Exception:
                continue
            if ve_layer not in wanted:
                continue
            try:
                # virtual entities are already unbound copies — bind them
                # directly (copy=False) so we keep the reference and can
                # repair the hatch pattern on the entity that actually lands
                # in the output (add_foreign_entity returns None).
    ...
    if skipped:
        result.warn(f"{skipped} nested wall entities of unsupported types "
                    f"could not be copied into the output")
    if verbatim == 0 and nested == 0:
        result.warn("wall_frame layers configured but no entities found on "
                    "them (top level or inside blocks)")


def _fix_hatch_pattern(entity) -> None:
    """Rebuild a copied HATCH's pattern definition from its own
    name/scale/angle attributes.

    The block-explode → foreign-copy chain can re-apply the pattern scale to
    the already-scaled baked definition (lines end up scale× too far apart).
    Re-deriving the definition from the entity's stored pattern name, scale
    and angle restores the source appearance.  Unknown/custom pattern names
    (not in ezdxf's predefined table) keep their copied definition untouched.
    """
    try:
        ...
        entity.set_pattern_fill(
            name,
            scale=float(entity.dxf.pattern_scale or 1.0),
            angle=float(entity.dxf.pattern_angle or 0.0),
            color=entity.dxf.color,
        )
    except Exception:
        # custom pattern name or API mismatch — keep the copied definition
        pass
